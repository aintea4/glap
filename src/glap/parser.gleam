...
fn parse_aux(
	...
	use <- bool.guard(when: args == [] && required_commands != [] && parser_settings.enforce_required, return: Error(error.MissingCommand(utils.strformat("missing required command: {}", [required_commands_str]))))
